...
export default function PharLabList() {
  ...
  useEffect(() => {
    ...
    if (filters.priceRange) {
      ...
      result = result.filter((provider) => {
        if (provider.healthcare_type !== "pharmacy" && provider.healthcare_type !== "laboratory") return true; // Skip non-doctor/nurse providers
  ...
  const healthcareTypes = ["laboratory", "pharmacy"]; // Available healthcare types
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 justify-items-center">
            {filteredProviders.map((provider) => {
              if (provider.healthcare_type === "pharmacy" || provider.healthcare_type === "laboratory") {
